<div class="ourProducts">
    

    <div  id="div-productos" class="products-content">

        <div class="title"  width=" 100%">
            <h3>NUESTROS PRODUCTOS</h3>
            <div class="subrayado"></div>
        </div>    

        <a class="productDiv s4" href="productos/marcos.php">            
            <div class="imgBackground" style="background-image: url(img/menu/marco.jpg)"></div>
            <div class="square" id="marcos">MARCOS</div>            
        </a>    

        <a class="productDiv s8" href="productos/raseros.php">            
            <div class="imgBackground" style=" background-image: url(img/menu/rasero.jpg)"></div>
            <div class="square" id="raseros">RASEROS</div>            
        </a>

        <a class="productDiv s4" href="productos/papel.php">        
            <div class="imgBackground" style="background-image: url(img/menu/papel.jpg)"></div>
            <div class="square" id="papeles">PAPELES</div>
        </a>

        <a class="productDiv s8" href="productos/tintas.php">            
            <div class="imgBackground" style="background-image: url(img/menu/tinta.jpg)"></div>
            <div class="square" id="tintas">TINTAS</div>            
        </a>    

        <!-- SECCION MOVIL -->
        <a href="productos/gorras.php" class="productDiv mov">
            <div class="imgBackground" style="background-image: url(img/menu/gorra.jpg)"></div>
            <div class="square" id="marcos">GORRAS</div>
        </a>

        <a href="productos/tintas-mas.php" class="productDiv mov">
            <div class="imgBackground" style="background-image: url(img/menu/tintas-y-mas.jpg)"></div>
            <div class="square" id="raseros">TINTAS Y MAS</div>
        </a>

        <a href="productos/taza.php"  class="productDiv mov">
            <div class="imgBackground" style="background-image: url(img/menu/taza.jpg)"></div>
            <div class="square" id="tintas">TAZAS</div>
        </a>  

        <a class="productDiv mov" href="productos/sellos.php">
            <div class="imgBackground" style="background-image: url(img/menu/sello.jpg)"></div>
            <div class="square" id="marcos">SELLOS</div>
        </a>

        <a class="productDiv mov" href="productos/vinil.php">
            <div class="imgBackground" style="background-image: url(img/menu/vinil.jpg)"></div>
            <div class="square" id="raseros">VINILES</div>
        </a>

        <a class="productDiv mov" href="productos/encuadernacion.php">
            <div class="imgBackground" style="background-image: url(img/menu/encuadernacion.jpg)"></div>
            <div class="square" id="raseros">ENCUADERNACIÓN</div>
        </a>

    </div>  

    

</div>   

<div id="div-products2" class="products-content products2 pc" >
        
        <a href="productos/gorras.php" class="productDiv s4">
            <div class="imgBackground" style="background-image: url(img/menu/gorra.jpg)"></div>
            <div class="square" id="marcos">GORRAS</div>
        </a>

        <a href="productos/tintas-mas.php" class="productDiv s8">
            <div class="imgBackground" style="background-image: url(img/menu/tintas-y-mas.jpg)"></div>
            <div class="square" id="raseros">TINTAS Y MAS</div>
        </a>

        <div class="productDiv s4 menu-links"  style="background: #e7e7e7">
            
            
        </div>

        <a href="productos/taza.php"  class="productDiv s8">
            <div class="imgBackground" style="background-image: url(img/menu/taza.jpg)"></div>
            <div class="square" id="tintas">TAZAS</div>
        </a>        
        
</div>    
            
            <!-- SEGUNDA SECCION DE PRODUCTOS -->
 
<div id="div-products3" class="products-content products3 pc">
    
    <a class="productDiv s4 sello" href="productos/sellos.php">
        <div class="imgBackground" style="background-image: url(img/menu/sello.jpg)"></div>
        <div class="square" id="marcos">SELLOS</div>
    </a>

    <div class="s8 finalProduct">
    
        <a class="pro2 productDiv sello" href="productos/vinil.php">
            <div class="imgBackground" style="background-image: url(img/menu/vinil.jpg)"></div>
            <div class="square" id="raseros">VINILES</div>
        </a>

        <a class=" pro2 productDiv sello" href="productos/encuadernacion.php">
            <div class="imgBackground" style="background-image: url(img/menu/encuadernacion.jpg)"></div>
            <div class="square" id="raseros">ENCUADERNACIÓN</div>
        </a>

    </div>


    
</div>  

